...
if ((digitalRead(L_S) == 1)&&(digitalRead(S_S) == 1)&&(digitalRead(R_S) == 0)){turnRight();} // rẻ trái
if ((digitalRead(L_S) == 1)&&(digitalRead(S_S) ==0)&&(digitalRead(R_S) == 0)) {turnRight();} // rẻ trái

if ((digitalRead(L_S) == 0)&&(digitalRead(S_S) == 1)&&(digitalRead(R_S) == 1)){turnLeft();} // rẻ phải
if ((digitalRead(L_S) == 0)&&(digitalRead(S_S) == 0)&&(digitalRead(R_S) == 1)){turnLeft();}// rẻ phải
...
if ((digitalRead(L_S) == 1)&&(digitalRead(S_S) == 1)&&(digitalRead(R_S) == 1)){Stop();} // stop
 ...
 if(distance < stop_distance) // nếu khoảng cách nhỏ hơn giới hạn 
 {
  ...
  delay(300);
  digitalWrite (motorA1,HIGH);// cho xe robot tien sang trái  
  digitalWrite(motorA2,LOW);                       
 ...
 digitalWrite (motorA1,LOW);// cho xe robot re phải 1 đoạn có thể thay đổi
  digitalWrite(motorA2,HIGH);                       
  digitalWrite (motorB1,LOW);
  digitalWrite(motorB2,LOW);
...
////////////////////////
 digitalWrite (motorA1,LOW); //cho xe robot re trái 
  digitalWrite(motorA2,HIGH);                       
  digitalWrite (motorB1,LOW);
  digitalWrite(motorB2,LOW);
   analogWrite (motorAspeed, 100);
